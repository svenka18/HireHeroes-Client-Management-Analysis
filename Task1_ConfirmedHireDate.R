...
dim(contact_log)
contact_log <- data.frame(contact_log$Id,contact_log$Dat_Initial_Assessment_was_Completed__c,contact_log$Date_assigned_to_staff__c,
                          contact_log$Date_Resume_Completed__c,contact_log$Date_turned_green__c,
                          contact_log$Confirmed_Hired_Date__c,contact_log$Attended_Day_1__c,contact_log$Attended_Day_2__c,contact_log$Compass_Participant__c,
...
contact_log$days_to_get_hired <- as.Date(as.character(contact_log$contact_log.Date_turned_green__c), format="%m/%d/%Y")-
  as.Date(as.character(contact_log$contact_log.Confirmed_Hired_Date__c), format="%m/%d/%Y")
...
dim(contact_log)
contact_log$contact_log.Attended_Day_1__c <- ifelse(is.na(contact_log$contact_log.Attended_Day_1__c),0,1)
contact_log$contact_log.Attended_Day_2__c <- ifelse(is.na(contact_log$contact_log.Attended_Day_2__c),0,1)
contact_log$contact_log.Compass_Participant__c <- ifelse(is.na(contact_log$contact_log.Compass_Participant__c),0,1)
...
contact_log$contact_log.Career_Opportunity_Day_Participant__c <- ifelse(is.na(contact_log$contact_log.Career_Opportunity_Day_Participant__c),0,1)
contact_log$contact_log.Created_LinkedIn_account__c <- ifelse(is.na(contact_log$contact_log.Created_LinkedIn_account__c),0,1)
...
################################
contact_log1 <- contact_log[complete.cases(contact_log), ]
